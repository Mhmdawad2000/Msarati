...
use Carbon\Carbon;
use App\Http\Resources\BusTrip\BusTripResource;
...
class BusTripController extends Controller
{
    ...
    public function AddBusTrip(Add_Bus_Trip $request)
    {
        ...
        return $this->returnData('Stored', 'bus_trip', new BusTripResource($bus_trip));
    }
    ...
    public function AddBusTripToArchive(Request $request)
    {
        ...
        return $this->returnData('Edited', 'bus_trip', new BusTripResource($bus_trip));
    }
    ...
    public function GetBusTripById(Request $request)
    {
        ...
        $bus_trip = BusTrip::find($request->id);
        if ($bus_trip) {
            return $this->returnCollection('bus_trip', new BusTripResource($bus_trip));
        } else {
    ...
    public function GetAllTheTripsToBus(Request $request)
    {
        ...
        return $this->returnCollection('bus_trips', new PagenateCollection(BusTrip::where('bus_id', $request->id)->get()->paginate(10)));
    }
}
